...
interface StylistSidebarNavProps {
  pendingInterestCount?: number;
  onNavigate?: () => void;
  className?: string;
}

export function StylistSidebarNav({
  pendingInterestCount = 0,
  onNavigate,
  className,
}: StylistSidebarNavProps) {
  const pathname = usePathname();
  const reduceMotion = useReducedMotion();

  return (
    <LayoutGroup id="stylist-dashboard-sidebar">
      <nav
        className={cn("flex flex-col gap-1", className)}
        aria-label="Stylist dashboard"
      >
        {STYLIST_NAV_ITEMS.map((item) => {
          const active = item.match(pathname);
          const Icon = item.icon;
          const badge =
            item.showInterestBadge && pendingInterestCount > 0
...
export function StylistSidebarBrand({
  onNavigate,
}: {
  onNavigate?: () => void;
}) {
